...
define([
  ...
  'backbone',
  'datepicker',
  '../views/registerUserSingleView',
  '../collections/registerUserCollection',
  '../models/registerUserFilterOptionModel',
  'text!../templates/registerUserRow.html',
  'text!../templates/registerUser_temp.html',
  'text!../templates/registerUserFilterOption_temp.html',
], function($,_, Backbone,datepicker,registerUserSingleView,registerUserCollection,registerUserFilterOptionModel,registerUserRowTemp,registerUserTemp,registerUserFilterTemp){
...
var registerUserView = Backbone.View.extend({
    ...
    initialize: function(options){
        ...
        readyState = true;
        this.render();
        filterOption = new registerUserFilterOptionModel();
        searchregisterUser = new registerUserCollection();
        searchregisterUser.fetch({headers: {
      ...
      this.collection = searchregisterUser;
    ...
    changeStatusListElement:function(e){
      ...
      $('#registerUserList input:checkbox').each(function(){
          if($(this).is(":checked"))
          {
            removeIds.push($(this).attr("data-userID"));
          }
      });
      ...
      if(idsToRemove == ''){
        ...
        $.ajax({
            url:APIPATH+'registerUserMaster/status',
            method:'POST',
    ...
    loadSubView:function(e){
        ...
        switch(show)
        {
          case "singleregisterUserData":{
            var userID = $(e.currentTarget).attr("data-userID");
            var registerUserSingleview = new registerUserSingleView({userID:userID,searchregisterUser:this});
            break;
          }
        }
    },
    resetSearch: function()
    {
        ...
        filterOption.set({curpage:0,userID:null,textval: null,textSearch:'firstname',status:'active',orderBy:'firstname',order:'ASC'});
        $(".multiOptionSel").removeClass("active");
        $("#textval").val("");
        $('#textSearch option[value=userID]').attr('selected','selected');
        this.filterSearch();
    },
    loaduser:function(){
    ...
    addOne: function(objectModel){
        var template = _.template(registerUserRowTemp);
        $("#registerUserList").append(template({registerUserDetails:objectModel}));
    },
    addAll: function(){
        $("#registerUserList").empty();
    ...
    filterRender:function(){
      ...
      if($('#filterData').length){
        ...
        var template = _.template(registerUserFilterTemp);
    ...
    render: function(){
        var template = _.template(registerUserTemp);
