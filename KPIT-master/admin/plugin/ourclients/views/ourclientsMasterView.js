...
define([
  ...
  'backbone',
  'datepicker',
  '../views/ourclientsMasterSingleView',
  '../collections/ourclientsMasterCollection',
  '../models/ourclientsMasterFilterOptionModel',
  'text!../templates/ourclientsMasterRow.html',
  'text!../templates/ourclientsMasterTemp.html',
  'text!../templates/ourclientsMasterFilterOptionTemp.html',
], function($,_, Backbone,datepicker,ourclientsMasterSingleView,ourclientsMasterCollection,ourclientsMasterFilterOptionModel,ourclientsMasterRowTemp,ourclientsMasterTemp,ourclientsMasterFilterTemp){
...
var ourclientsMasterView = Backbone.View.extend({
    ...
    initialize: function(options){
        ...
        readyState = true;
        this.render();
        filterOption = new ourclientsMasterFilterOptionModel();
        searchcategoryMaster = new ourclientsMasterCollection();
        searchcategoryMaster.fetch({headers: {
      ...
      this.collection = searchcategoryMaster;
    ...
    changeStatusListElement:function(e){
      ...
      var action = "delete";
      $('#clientMasterList input:checkbox').each(function(){
          if($(this).is(":checked"))
          {
            removeIds.push($(this).attr("data-clientsid"));
          }
      });
      ...
      if(idsToRemove == ''){
        ...
        $.ajax({
            url:APIPATH+'ourclientssave/status',
            method:'POST',
    ...
    loadSubView:function(e){
        ...
        switch(show)
        {
          case "singleclientsData":{
            var clientsID = $(e.currentTarget).attr("data-clientsID");
            var ourclientsMasterSingleview = new ourclientsMasterSingleView({clientsID:clientsID,searchclientsMaster:this});
            break;
          }
        }
    },
    resetSearch: function()
    {
        ...
        filterOption.set({curpage:0,categoryID:null,textval: null,textSearch:'categoryName',status:'active',orderBy:'categoryName',order:'ASC'});
        $(".multiOptionSel").removeClass("active");
        $("#textval").val("");
        $('#textSearch option[value=categoryID]').attr('selected','selected');
        this.filterSearch();
    },
    loaduser:function(){
    ...
    addOne: function(objectModel){
        var template = _.template(ourclientsMasterRowTemp);
        $("#clientMasterList").append(template({clientsDetails:objectModel}));
    },
    addAll: function(){
        $("#clientMasterList").empty();
    ...
    filterRender:function(){
      ...
      if($('#filterData').length){
        ...
        var template = _.template(categoryMasterFilterTemp);
    ...
    render: function(){
        var template = _.template(ourclientsMasterTemp);
