$(document).ready(function(){
            ...
            onSucess:function(data){
                try{
                    if(data.status == "File uploaded"){
                        ...
                        $("#icard-file").attr("href", base_url+"uploads/student_icards/"+data.filename);
    ...
    $("#registerUserEduDetails").validate({
        ...
        messages: {
            ...
            if($("#identityCard").val()==""){
                $("#reg-icard").append('<div class="error">Upload College Identity Card</div>');
                return false;
            }
            this.valid();
            ...
            success:function(res){
                ...
                if(res.flag == "S"){
                    var msg = alertify.success('Default message');
                  msg.delay(3).setContent(res.msg);
                  msg.ondismiss = function(){  window.location = base_url+"verify";  };
    ...
    $(".replaceState").on("change",function(e){
        ...
        $.ajax({
            ...
            success:function(res){
            res = JSON.parse(res);
            $('#collegel').find('option').remove().end().append('<option value="">Select College</option>');
            ...
            if(res.flag == "S"){
                ...
                if(res.data!="")
                {
                    res.data.forEach(function(opt){
                     $('#collegel').append("<option value='"+opt.college_id+"'>"+opt.college_name+"</option>");
                    })
                }
    ...
    $(".changeState").on("change",function(e){
        ...
        if(valuetxt !=""){
            $.ajax({
                ...
                success:function(res){
                res = JSON.parse(res);
                $('#collegeList').find('option').remove().end().append('<option value="">Select</option>');
                $('#cityList').val("");
                if(res.flag == "F")
                {
                ...
                if(res.flag == "S"){
                    ...
                    {
                        res.data.forEach(function(opt){
                        $('#collegeList').append("<option value='"+opt.college_id+"'>"+opt.college_name+"</option>");
                        })
                    }
