...
function selectProject(projectID){
    ...
    if($("#project-"+projectID).html() == "Select"){
        ...
        $("#remove-msg").html("Are you sure you want to select the project "+$("#project_link_"+projectID).html()+"?");
        $("#removeConfirmModal").modal("show");
        $("#btnRemoveConfirm").attr("pid", projectID);
    }else{
        $("#confirm-title").html("Project Removal");
        $("#remove-msg").html("Are you sure you want to remove the "+$("#project_link_"+projectID).html()+" from the inbox?");
        $("#removeConfirmModal").modal("show");
        $("#btnRemoveConfirm").attr("pid", projectID);
            ...
            if(res !=''){
                ...
                if(res.flag == "S"){
                    if($("#project-"+projectID).html() == "Select"){
                        ...
                        $("#selection-msg").html("The selected idea "+$("#project_link_"+projectID).html()+" will now be available in the inbox");
                        alertify.success("The selected idea "+$("#project_link_"+projectID).html()+" will now be available in the inbox");
                        ...
                        alertify.success("The selected idea "+$("#project_link_"+projectID).html()+" removed from inbox");
                        $("#selection-msg").html("The selected idea "+$("#project_link_"+projectID).html()+" removed from inbox");
                    }
...
$(document).ready(function() {
    ...
    $("#evaluators-details").validate({
        ...
        submitHandler: function(form) {
            ...
            alertify.confirm('', 'Are you sure want to submit the score?', function(){ 
            //return "";                
...
function filterProjectList(subCatIds){
    $('#example').DataTable( {
        destroy: true,
      "ajax": base_url+'evaluator/dashboard?ajax=1&self_list='+self_list+'&subCtIds='+subCatIds
  } );
}