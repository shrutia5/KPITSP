...
class Reports extends CI_Controller {
    ...
    public function index() {
        ...
        switch ($reportType) {
            ...
            case 'all_report': {
                    ...
                    switch ($allrep) {
                        case 'state_wise': {
                                ...
                                $wherec = array("status" => " ='active'", "country_id" => " ='101'");
            ...
            case 'statistics2': {
                    ...
                    $numberofRegistrations = $this->CommonModel->getNumberofRegistrations();
                    $regCount = $numberofRegistrations[0]->count;

                    $numberofNewRegistrations = $this->CommonModel->getNumberofNewRegistrations();
                    $newregCount = $numberofNewRegistrations[0]->count;

                    $numberOfIdeas = $this->CommonModel->getNumberofIdeasSubmitted();
                    $ideaCount = $numberOfIdeas[0]->count;

                    $numberOfStates = $this->CommonModel->getNumberofStates();
                    $stateCount = $numberOfStates[0]->count;

                    $numberOfColleges = $this->CommonModel->getNumberofColleges();
                    $collegeCount = $numberOfColleges[0]->count;

                    $numberOfPremColleges = $this->CommonModel->getNumberofPremColleges();
                    $premCount = $numberOfPremColleges[0]->count;
                    ...
                    $data['newregCount'] = $newregCount;
                    $data['ideaCount'] = $ideaCount;
    ...
    public function pdfexcelReport($report_type, $printDetails1, $allRepType, $reportType) {
        ...
        switch ($reportType) {
            case 'list_of_reg': {
                    foreach ($printDetails1['otherPage']['list'] as $key => $value) {
                        ...
                        if ($value->is_top_100 == "1") {
                            $printDetails[$i]['clgtop100'] = "Yes";
                        } else {
                            $printDetails[$i]['clgtop100'] = "No";
                        }
                        // $printDetails[$i]['clgtop100'] = $value->numberOfideafinal;
                        if ($value->is_premier == "1") {
                            $printDetails[$i]['ispremier'] = "Yes";
                        } else {
                            $printDetails[$i]['ispremier'] = "No";
                        }
                        //$printDetails[$i]['ispremier'] = $value->numberOfideafinal;
                        if ($value->phaseOneDataSubmited == "1") {
                            $printDetails[$i]['ideasubmitted'] = "Yes";
                        } else {
                            $printDetails[$i]['ideasubmitted'] = "No";
                        }
                        //$printDetails[$i]['ideasubmitted'] = $value->numberOfideafinal;
                        $printDetails[$i]['emailVerify'] = $value->verify_email;
                        if ($value->phaseTwoDataSubmited == "1") {
                            $printDetails[$i]['ideaphase2'] = "Yes";
                        } else {
                            $printDetails[$i]['ideaphase2'] = "No";
                        }
                        //$printDetails[$i]['ideaphase2'] = $value->numberOfideafinal;
                        if ($value->phaseTwoStatus == "Approved" && $value->currentPhase == "3") {
                            $printDetails[$i]['ideatop100'] = "Yes";
                        } else {
                            $printDetails[$i]['ideatop100'] = "No";
                        }
                        //$printDetails[$i]['ideatop100'] = $value->numberOfideafinal;
                        //$printDetails[$i]['ideafinale'] = $value->numberOfideafinal;
                        if ($value->phaseThreeStatus == "50") {
                            $printDetails[$i]['ideafinale'] = "Yes";
                        }
                        $i++;
                    }
        ...
        if ($report_type == "pdf") {
            ...
            $pdfFilePath = $this->load->view("prose/datapdf", $data, true);
            //load mPDF library
