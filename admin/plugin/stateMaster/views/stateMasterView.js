...
define([
  ...
  'backbone',
  'datepicker',
  '../views/stateMasterSingleView',
  '../collections/stateMasterCollection',
  '../models/stateMasterFilterOptionModel',
  'text!../templates/stateMasterRow.html',
  'text!../templates/stateMasterTemp.html',
  'text!../templates/stateMasterFilterOptionTemp.html',
], function($,_, Backbone,datepicker,stateMasterSingleView,stateMasterCollection,stateMasterFilterOptionModel,stateMasterRowTemp,stateMasterTemp,stateMasterFilterTemp){
...
var stateMasterView = Backbone.View.extend({
    ...
    initialize: function(options){
        ...
        filterOption = new stateMasterFilterOptionModel();
        searchstateMaster = new stateMasterCollection();
        searchstateMaster.fetch({headers: {
      ...
      this.collection = searchstateMaster;
    ...
    changeStatusListElement:function(e){
      ...
      $('#stateMasterList input:checkbox').each(function(){
          if($(this).is(":checked"))
          {
            removeIds.push($(this).attr("data-stateID"));
          }
      });
      ...
      if(idsToRemove == ''){
        ...
        $.ajax({
            url:APIPATH+'stateMaster/status',
            method:'POST',
    ...
    loadSubView:function(e){
        ...
        switch(show)
        {
          case "singlestateMasterData":{
            var stateID = $(e.currentTarget).attr("data-stateID");
            var stateMasterSingleview = new stateMasterSingleView({stateID:stateID,searchstateMaster:this});
            break;
          }
        }
    },
    resetSearch: function()
    {
        ...
        filterOption.set({curpage:0,stateID:null,textval: null,textSearch:'stateName',status:'active',orderBy:'stateName',order:'ASC'});
        $(".multiOptionSel").removeClass("active");
        $("#textval").val("");
        $('#textSearch option[value=stateID]').attr('selected','selected');
        this.filterSearch();
    },
    loaduser:function(){
    ...
    addOne: function(objectModel){
        var template = _.template(stateMasterRowTemp);
        $("#stateMasterList").append(template({stateMasterDetails:objectModel}));
    },
    addAll: function(){
        $("#stateMasterList").empty();
    ...
    filterRender:function(){
      ...
      if($('#filterData').length){
        ...
        var template = _.template(stateMasterFilterTemp);
    ...
    render: function(){
        var template = _.template(stateMasterTemp);
