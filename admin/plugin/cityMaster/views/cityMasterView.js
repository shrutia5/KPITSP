...
define([
  ...
  'backbone',
  'datepicker',
  '../views/cityMasterSingleView',
  '../collections/cityMasterCollection',
  '../models/cityMasterFilterOptionModel',
  '../../stateMaster/collections/stateMasterCollection',
  'text!../templates/cityMasterRow.html',
  'text!../templates/cityMasterTemp.html',
  'text!../templates/cityMasterFilterOptionTemp.html',
], function($,_, Backbone,datepicker,cityMasterSingleView,cityMasterCollection,cityMasterFilterOptionModel,stateMasterCollection,cityMasterRowTemp,cityMasterTemp,cityMasterFilterTemp){
...
var cityMasterView = Backbone.View.extend({
    ...
    initialize: function(options){
        ...
        readycity = true;
        this.render();
        filterOption = new cityMasterFilterOptionModel();
        searchcityMaster = new cityMasterCollection();
        
        searchcityMaster.fetch({headers: {
          'contentType':'application/x-www-form-urlencoded','SadminID':$.cookie('authid'),'token':$.cookie('_bb_key'),'Accept':'application/json'
        },error: selfobj.onErrorHandler,type:'post',data:filterOption.attributes}).done(function(res){
          
          if(res.statusCode == 994){app_router.navigate("logout",{trigger:true});}
          $(".profile-loader").hide();
        setPagging(res.paginginfo,res.loadcity,res.msg);   
        //selfobj.render();
        });
      ...
      this.collection = searchcityMaster;
    ...
    changeStatusListElement:function(e){
      ...
      $('#cityMasterList input:checkbox').each(function(){
          if($(this).is(":checked"))
          {
            removeIds.push($(this).attr("data-cityID"));
          }
      });
    ...
    loadSubView:function(e){
        ...
        switch(show)
        {
          case "singlecityMasterData":{
            var cityID = $(e.currentTarget).attr("data-cityID");
            var cityMasterSingleview = new cityMasterSingleView({cityID:cityID,searchcityMaster:this});
            break;
          }
        }
    },
    resetSearch: function()
    {
        ...
        filterOption.set({curpage:0,cityID:null,textval: null,textSearch:'cityName',status:'active',orderBy:'cityName',order:'ASC'});
        $(".multiOptionSel").removeClass("active");
        $("#textval").val("");
        $('#textSearch option[value=cityID]').attr('selected','selected');
        this.filterSearch();
    },
    loaduser:function(){
    ...
    addOne: function(objectModel){
        var template = _.template(cityMasterRowTemp);
        console.log("Div");
        console.log($("#cityMasterList").length);
        $("#cityMasterList").append(template({cityMasterDetails:objectModel}));
    },
    addAll: function(){
      $("#cityMasterList").append("<tr><td>test</td></tr>")
        $("#cityMasterList").empty();
    ...
    filterRender:function(){
      ...
      if($('#filterData').length){
        ...
        var template = _.template(cityMasterFilterTemp);
         ////this.$el.html();
         console.log("state list");
         //console.log(this.stateMaster.models);
        $("#modalBody").append(template({stateList:this.stateMaster.models}));
    ...
    render: function(){
        var template = _.template(cityMasterTemp);
