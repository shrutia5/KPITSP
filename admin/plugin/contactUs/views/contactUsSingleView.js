...
], function($,_, Backbone,validate,inputmask,datepicker,contactUsSingleModel,contactUsTemp){
...
var contactUsView = Backbone.View.extend({
    ...
    initialize: function(options){
        var selfobj = this;
        $(".modelbox").hide();
        scanDetails = options.searchcontactUs;
        $('#faqData').remove();
        $(".popupLoader").show();
      
        this.model = new contactUsSingleModel();
        if(options.contactUsID != ""){
          this.model.set({contactUsID:options.contactUsID});
    ...
    saveFaqDetails: function(e){
      e.preventDefault();
      var contactUsID = this.model.get("contactUsID");
